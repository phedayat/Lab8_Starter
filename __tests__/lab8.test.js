describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    expect(res).toBe("Remove from Cart");
  }, 2500);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const res = await page.$$eval("product-item", prodItems => {
      let allRes = true;
      prodItems.forEach(prod => {
        let shadow = prod.shadowRoot;
        let btn = shadow.querySelector("button");
      ...
      let cartCount = document.querySelector("#cart-count").innerHTML;
      return [cartCount, allRes];
    });
